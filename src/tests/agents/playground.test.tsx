...
describe('AgentPlaygroundSimulator', () => {
  beforeAll(() => {
    ;(supabase.rpc as any).mockImplementation(async (fnName: string, params: any) => {
      if (fnName === 'search_unified_context') {
        if (params?.p_tenant_id === '00000000-0000-0000-0000-000000000001') {
          return {
            data: [
  ...
  test('renderiza el chat con mensaje de bienvenida', () => {
    ...
    render(<AgentPlaygroundSimulator workspace={workspace} onUpdateWorkspace={mockOnUpdateWorkspace} />)
  ...
  test('muestra inspector de prompt compilado', () => {
    const workspace = createMockWorkspace(QAWAY_UUID)
    render(<AgentPlaygroundSimulator workspace={workspace} onUpdateWorkspace={mockOnUpdateWorkspace} />)
    
    fireEvent.click(screen.getByText('Inspeccionar Prompt Compilado (3 Capas)'))
    expect(screen.getByText('CAPA 0')).toBeInTheDocument()
  })

  test('cambia tenant y actualiza contexto', async () => {
    const { rerender } = render(<AgentPlaygroundSimulator workspace={createMockWorkspace(QAWAY_UUID)} onUpdateWorkspace={mockOnUpdateWorkspace} />)
    
    // Simular cambio de tenant via props
    rerender(<AgentPlaygroundSimulator workspace={createMockWorkspace(CORAVET_UUID)} onUpdateWorkspace={mockOnUpdateWorkspace} />)
    
    expect(screen.getByText('CoraVet Clínica Veterinaria')).toBeInTheDocument()
  })
})
...
describe('ContextEngine integration', () => {
  test('buildTenantContext usa tenantId real (no workspace.id)', async () => {
    ...
    const ctx = await import('@/pages/5-qaway-hub/2-Agentes/services/contextEngine').then(m => m.buildTenantContext('00000000-0000-0000-0000-000000000001', workspace, 'conv-1', []))
