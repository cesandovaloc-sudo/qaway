    ...
    <AnimatePresence>
      {toast && (
        <motion.div
          ...
          exit={{ opacity: 0, y: 10 }}
          className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 bg-white border border-slate-200 text-slate-900 font-bold text-xs rounded-2xl px-5 py-3.5 shadow-2xl shadow-slate-300/50"
        >
          {toast.type === 'error' ? (
...
function AgendaInnerRoutes() {
  return (
    <div className="min-h-screen bg-[#f5f6fe] text-slate-900 font-sans selection:bg-indigo-500 selection:text-white">
      <Routes>
