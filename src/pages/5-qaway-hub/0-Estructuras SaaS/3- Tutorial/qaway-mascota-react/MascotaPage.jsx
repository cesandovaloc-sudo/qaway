        ...
        {happy && (
          <>
            <motion.span
              ...
              animate={{ opacity: [0, 1, 0], scale: [0.4, 1.15, 1], y: [8, -28, -42] }}
            ...
            <motion.span
              ...
              initial={{ opacity: 0, scale: 0.4, y: 8 }}
              animate={{ opacity: [0, 1, 0], scale: [0.4, 1.15, 1], y: [8, -18, -35] }}
              transition={{ duration: 0.75, delay: 0.08 }}
            >
              *
            </motion.span>
            <motion.span
              className="spark spark-c"
              initial={{ opacity: 0, scale: 0.4, y: 8 }}
              animate={{ opacity: [0, 1, 0], scale: [0.4, 1.15, 1], y: [8, -30, -48] }}
...
export default function MascotaPage() {
  ...
  return (
    <div className="qaway-mascota-scope min-h-screen text-[#f8fafc] bg-[#090d12] antialiased">
      <style>{`
        .qaway-mascota-scope {
          font-family: Inter, system-ui, sans-serif;
        }
        .qaway-mascota-scope .page-canvas {
          min-height: 100vh;
          background:
            radial-gradient(circle at 75% 48%, rgba(255,75,11,.12), transparent 28%),
            radial-gradient(circle at 15% 20%, rgba(255,255,255,.045), transparent 25%),
            #090d12;
        }
        .qaway-mascota-scope .top-bar {
          height: 76px;
          padding: 0 5vw;
          display: flex;
          align-items: center;
        ...
        .qaway-mascota-scope .content-grid {
        ...
        .qaway-mascota-scope .stage {
          ...
          background:
            ...
            #0d131a;
          box-shadow: inset 0 1px 0 rgba(255,255,255,.04), 0 30px 90px rgba(0,0,0,.35);
        }
        .qaway-mascota-scope .stage-glow {
          position: absolute;
          width: 370px;
          height: 370px;
          border-radius: 50%;
          background: radial-gradient(circle, rgba(255,75,11,.16), transparent 68%);
          filter: blur(8px);
        }
        .qaway-mascota-scope .mascot-shell {
          position: relative;
          width: min(480px, 72%);
          aspect-ratio: 1;
          z-index: 2;
        }
        .qaway-mascota-scope .mascot {
          width: 100%;
          height: 100%;
          display: block;
          overflow: visible;
        }
        .qaway-mascota-scope .mascot-orbit {
          position: absolute;
          inset: 10%;
          border: 1px solid rgba(255,75,11,.16);
          border-radius: 50%;
        }
        .qaway-mascota-scope .spark {
          position: absolute;
          color: #ff4b0b;
          font-size: 36px;
          font-weight: 800;
        ...
        .qaway-mascota-scope .spark-a { left: 12%; top: 19%; }
        .qaway-mascota-scope .spark-b { right: 16%; top: 14%; }
        .qaway-mascota-scope .spark-c { right: 7%; top: 42%; font-size: 28px; }
        .qaway-mascota-scope .label-pill {
          position: absolute;
          bottom: 34px;
          left: 50%;
        ...
        <header className="top-bar">
          <div className="flex items-center gap-4">
            ...
            <div className="text-lg font-extrabold tracking-tight">
              <span className="inline-block w-2 h-2 mr-2 bg-[#ff4b0b] rounded-full align-middle" />
              Qaway <span className="text-[#ff4b0b]">Lab</span>
            </div>
          </div>
          <span className="text-slate-400 text-xs font-mono border border-white/10 rounded-full px-3 py-1">
            React 19 + Framer Motion + SVG
          </span>
        </header>

        <main className="content-grid">
          <div className="space-y-6">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#ff4b0b]/10 border border-[#ff4b0b]/25 text-[#ff4b0b] text-[11px] font-bold uppercase tracking-widest">
              <Sparkles className="w-3.5 h-3.5" />
              <span>Mascota Asistente & Onboarding</span>
            </div>

            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-[1.05] tracking-tight text-white">
              Una mascota que <span className="text-[#ff4b0b]">reacciona</span> a la interfaz.
            </h1>

            <p className="text-base sm:text-lg text-slate-300/80 leading-relaxed max-w-lg font-normal">
              Demostración interactiva de la mascota vectorial desarrollada como componente React para guiar los
              flujos de Onboarding y tutoriales de Qaway Lab.
            </p>

            <div className="pt-2 flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <button
                type="button"
                onClick={() => setMood(mood === 'normal' ? 'happy' : 'normal')}
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#ff4b0b] px-6 py-3.5 text-sm font-bold uppercase tracking-wider text-white shadow-[0_10px_28px_rgba(255,75,11,0.28)] hover:brightness-110 hover:-translate-y-0.5 transition-all cursor-pointer"
              >
                <span>{mood === 'normal' ? 'Hacer feliz a la mascota' : 'Volver a estado normal'}</span>
              </button>

              <div className="flex items-center gap-2.5 text-xs text-slate-400 font-mono">
                <span
                  className={`w-2.5 h-2.5 rounded-full transition-all ${
                    mood === 'happy' ? 'bg-[#ff4b0b] shadow-[0_0_12px_rgba(255,75,11,0.9)]' : 'bg-slate-600'
                  }`}
                />
                <span>
                  Estado: <strong className="text-white">{mood === 'happy' ? 'Feliz (Saltando)' : 'Normal (Atento)'}</strong>
                </span>
              </div>
            </div>

            <div className="rounded-xl border border-white/10 bg-white/5 p-4 text-xs text-slate-400 leading-relaxed">
              <span className="font-bold text-white block mb-1">Mecánica técnica:</span>
              Al cambiar el estado a <code className="text-[#ff7a18]">happy</code>, se sincronizan las curvas cúbicas de salto en el cuerpo, el flameo en la cola, la rotación de los brazos, la apertura de ojos/boca con <code className="text-[#ff7a18]">AnimatePresence</code> y la emisión de destellos.
            </div>
          </div>
          ...
          <div className="stage">
            ...
            <div className="label-pill">
              {mood === 'happy' ? '¡Vamos con todo!' : 'Qaway Assistant (En espera)'}
            </div>
          </div>
        </main>
      </div>
