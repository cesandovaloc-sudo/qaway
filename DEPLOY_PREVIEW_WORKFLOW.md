...
## CuÃƒÂ¡ndo una pagina puede ir online
...
## Workflow adicional: switch de visibilidad + subida manual

Este es el flujo adicional que se aprobó para no olvidar los pasos cuando la IA solo ajusta que rutas salen al publico.

### Objetivo

- dejar todo visible para ti en `dev`
- controlar solo la salida publica con un switch
- evitar tocar manualmente varias partes del router en cada deploy simple

### Archivo central

El switch vive en:

`C:\LEO\EMPRESAS\QAWAY LAB\1-Web-Qaway\1-Frontend\src\config\siteVisibility.js`

### Regla de funcionamiento

- en `npm run dev`, se ve todo
- en build publico, solo salen las rutas marcadas en `true`

### Workflow exacto

1. la IA modifica `siteVisibility.js`
2. tu revisas localmente con `npm run dev`
3. tu ejecutas el build publico
4. tu copias manualmente el contenido de `dist`
5. tu lo subes manualmente a `public_html`
6. tu verificas la web

### Comando exacto de build publico
...
Subir manualmente el contenido de:

`C:\LEO\EMPRESAS\QAWAY LAB\1-Web-Qaway\1-Frontend\dist`

hacia:

`public_html`

### Que valida la IA antes

Antes de decirte que hagas build o subida manual, la IA debe confirmar:

1. rama actual real
2. archivo `siteVisibility.js` correcto
3. que `dev` sigue mostrando todo
4. que el build publico filtrara solo lo aprobado
