...
serve(async (req: Request) => {
  ...
  if (req.method === 'GET') {
    ...
    if (mode === 'subscribe' && token === VERIFY_TOKEN) {
      console.log('[WABA Webhook] Webhook validado exitosamente por Meta.')
  ...
  if (req.method === 'POST') {
    try {
      ...
      if (!isAuthentic) {
        console.error('[WABA Webhook] Firma x-hub-signature-256 no coincide con APP_SECRET.')
              ...
              for (let i = 0; i < messages.length; i++) {
                ...
                if (existingLeads && existingLeads.length > 0) {
                  ...
                  if (!alreadyExists) {
                    ...
                    console.log(`[WABA Webhook] Lead ${senderPhone} actualizado. Handoff: ${isHumanRequested}, CTWA: ${Boolean(referral)}`)
                  } else {
                    console.log(`[WABA Webhook] Mensaje duplicado omitido (${wamid})`)
                  }
                } else {
                  ...
                  console.log(`[WABA Webhook] Nuevo lead creado ${senderPhone}. CTWA: ${Boolean(referral)}`)
                }
              }
            }
          }
      ...
      console.error('[WABA Webhook] Error procesando webhook:', error)
