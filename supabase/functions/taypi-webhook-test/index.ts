...
Deno.serve(async (req) => {
  ...
  try {
    if (!TAYPI_PUBLIC_KEY || !TAYPI_SECRET_KEY) {
      console.error("Faltan credenciales TAYPI TEST");
    ...
    if (!TAYPI_WEBHOOK_SECRET) {
      console.error("Falta TAYPI_TEST_WEBHOOK_SECRET");
    ...
    console.log("TAYPI WEBHOOK TEST:", event);
    ...
    if (!Number.isFinite(eventAmountCents) || orderTotalCents !== eventAmountCents) {
      console.error("MONTO DISCREPANTE TAYPI:", {
    ...
    console.log("TAYPI TEST: Pago confirmado y orden pagada con éxito:", {
      orderId,
      paymentId,
      amount: eventAmount,
    });
    ...
    console.error("TAYPI WEBHOOK TEST ERROR:", error);
    return new Response(
      JSON.stringify({
        received: false,
