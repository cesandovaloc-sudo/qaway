...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. Crear preferencia en Mercado Pago (Sandbox)
    const cuerpo = {
      items,
      ...
      notification_url: webhookBase && anonKey
        ? `${webhookBase}/functions/v1/mercadopago-webhook-test?apikey=${anonKey}`
        : undefined,
    ...
    console.error('[mercadopago-pago-test]', err)
