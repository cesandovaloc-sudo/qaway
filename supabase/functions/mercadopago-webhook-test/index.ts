...
Deno.serve(async (req) => {
  ...
  if (dataId === '123456' || (evento?.action === 'order.processed' && dataId === '123456')) {
    console.log('[Webhook MP Test] Ping de prueba verificado exitosamente')
    return json({ ok: true, message: 'Webhook test endpoint verified successfully' }, 200)
  }

  const secret = Deno.env.get('MERCADOPAGO_TEST_WEBHOOK_SECRET') ?? ''

  // 1. R3: Verificar firma criptográfica
  const valida = await firmaMpValida(req, dataId, secret)
  if (!valida) {
    console.warn('[Webhook MP Test] Firma inválida para data.id:', dataId)
  ...
  // 2. Consultar pago en API de Mercado Pago (Sandbox)
  const token = Deno.env.get('MERCADOPAGO_TEST_ACCESS_TOKEN') ?? ''
  ...
  if (Math.abs(Number(intentoValido.amount) - montoPagado) > 0.01) {
    console.error('[Webhook MP Test] R5 MONTO DISCREPANTE:', { esperado: intentoValido.amount, recibido: montoPagado })
  ...
  console.log('[Webhook MP Test] Pedido pagado:', intentoValido.order_id)
