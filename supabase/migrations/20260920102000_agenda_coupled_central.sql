-- ============================================================
-- ACOPLE AGENDA EN CENTRAL (run-2): tablas propias + tenant
-- Agenda vive en la web: sus tablas no existían en central y el
-- frontend ya habla al cliente central. Tablas propias (una cita
-- no es un pedido), puente por businesses.tenant_id → tenants.
...
create index if not exists businesses_tenant_idx on public.businesses (tenant_id);
...
-- Staff: su marca (owner + tenant coincidente con el usuario)
drop policy if exists "businesses_tenant_owner" on public.businesses;
create policy "businesses_tenant_owner" on public.businesses
  for all using (
    public.is_admin()
    or (owner_id = auth.uid() and (tenant_id = public.get_auth_tenant_id() or tenant_id is null))
  )
  with check (
    public.is_admin()
    or (owner_id = auth.uid() and (tenant_id = public.get_auth_tenant_id() or tenant_id is null))
...
drop policy if exists "agenda_child_owner" on public.event_types;
create policy "agenda_child_owner" on public.event_types
  for all using (
    public.is_admin()
    or exists (select 1 from public.businesses b where b.id = event_types.business_id and (b.owner_id = auth.uid() or b.tenant_id = public.get_auth_tenant_id()))
  )
  with check (
    public.is_admin()
    or exists (select 1 from public.businesses b where b.id = event_types.business_id and (b.owner_id = auth.uid() or b.tenant_id = public.get_auth_tenant_id()))
...
drop policy if exists "agenda_child_owner" on public.schedules;
create policy "agenda_child_owner" on public.schedules
  for all using (
    public.is_admin()
    or exists (select 1 from public.businesses b where b.id = schedules.business_id and (b.owner_id = auth.uid() or b.tenant_id = public.get_auth_tenant_id()))
  )
  with check (
    public.is_admin()
    or exists (select 1 from public.businesses b where b.id = schedules.business_id and (b.owner_id = auth.uid() or b.tenant_id = public.get_auth_tenant_id()))
...
drop policy if exists "agenda_child_owner" on public.availability_exceptions;
create policy "agenda_child_owner" on public.availability_exceptions
  for all using (
    public.is_admin()
    or exists (select 1 from public.businesses b where b.id = availability_exceptions.business_id and (b.owner_id = auth.uid() or b.tenant_id = public.get_auth_tenant_id()))
  )
  with check (
    public.is_admin()
    or exists (select 1 from public.businesses b where b.id = availability_exceptions.business_id and (b.owner_id = auth.uid() or b.tenant_id = public.get_auth_tenant_id()))
...
drop policy if exists "agenda_bookings_owner" on public.bookings;
create policy "agenda_bookings_owner" on public.bookings
  for all using (
    public.is_admin()
    or exists (select 1 from public.businesses b where b.id = bookings.business_id and (b.owner_id = auth.uid() or b.tenant_id = public.get_auth_tenant_id()))
  )
  with check (
    public.is_admin()
    or exists (select 1 from public.businesses b where b.id = bookings.business_id and (b.owner_id = auth.uid() or b.tenant_id = public.get_auth_tenant_id()))
...
-- Anon: lectura pública + reserva solo eventos gratuitos
drop policy if exists "anon_read_business" on public.businesses;
create policy "anon_read_business" on public.businesses for select to anon using (true);

drop policy if exists "anon_read_active_events" on public.event_types;
...
drop policy if exists "anon_read_exceptions" on public.availability_exceptions;
...
create policy "anon_insert_booking" on public.bookings
  for insert to anon with check (
    ...
    and exists (
      ...
      where e.id = event_type_id and e.is_active = true and coalesce(e.price, 0) = 0
    )
  );
...
-- Vista slots + RPC por token (sin PII)
...
grant select on public.businesses, public.event_types, public.schedules, public.availability_exceptions to anon;
grant all on public.businesses, public.event_types, public.schedules, public.availability_exceptions, public.bookings, public.reminders to authenticated;
grant all on public.businesses, public.event_types, public.schedules, public.availability_exceptions, public.bookings, public.reminders to service_role;
